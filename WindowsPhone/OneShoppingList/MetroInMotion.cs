...
namespace MetroInMotionUtils
{
    public static class MetroInMotion
    {
          ...
          DependencyPropertyChangedEventArgs args)
        {
            ...
            // associate with the source element
        ...
        private static void OnIsPivotAnimatedChanged(DependencyObject d, DependencyPropertyChangedEventArgs args)
        {
            ...
            // this will hold all items linked in the long list selector (if the frameworkElement is a LongListSelector)
            // else it will remain null, indicating that the frameworkElement is NOT a LongListSelector
            List<FrameworkElement> llsLinkedItems = null;
            if (frameworkElement is LongListSelector)
            {
            ...
            frameworkElement.Loaded += (s2, e2) =>
              {
                  if (!isRegistrationDone)
                  {
                      isRegistrationDone = true;
                      // locate the pivot control that this lls is within
                      Pivot pivot = frameworkElement.Ancestors<Pivot>().Single() as Pivot;

                      // and its index within the pivot
                      PivotItem pivotItem = frameworkElement.Ancestors<PivotItem>().Single() as PivotItem;
                      int pivotIndex = pivot.Items.IndexOf(pivotItem);
                      if (pivotIndex < 0)
                          pivotIndex = pivot.Items.IndexOf(pivotItem.DataContext);
                      ...
                      pivot.ManipulationCompleted += (s, e) =>
                        {
                            ...
                            // identify the items in view:
                            var items = llsLinkedItems;
                            if (items == null && frameworkElement is ItemsControl)
                            {
                                items = (frameworkElement as ItemsControl).GetItemsInView().ToList();
                            }
        ...
        /// a Storyboard which will animate the PlaneProjection to 'peel' the target
        /// from the screen.
        /// </summary>
    ...
    public class ItemFlyInAndOutAnimations
    {
        ...
        public ItemFlyInAndOutAnimations(double x, double y)
        {
            TargetX = x;
            TargetY = y;
        ...
        public void ItemFlyOut(FrameworkElement element, Action action)
        {
            ...
            // animate the X position
            var db = CreateDoubleAnimation(_targetElementPosition.X, TargetX,
                new SineEase() { EasingMode = EasingMode.EaseOut },
                _targetElementClone, Canvas.LeftProperty, _flyOutSpeed);
            sb.Children.Add(db);

            // animate the Y position
            db = CreateDoubleAnimation(_targetElementPosition.Y, TargetY,
